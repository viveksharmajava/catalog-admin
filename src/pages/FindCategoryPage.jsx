...
import { findCategories } from '../api/catalogApi';
...
const defaultForm = {
  productCategoryId: { ...emptyCriteria },
  categoryName: { ...emptyCriteria },
...
const columns = [
  { key: 'productCategoryId', label: 'Category ID' },
  { key: 'productCategoryTypeId', label: 'Category Type' },
  { key: 'primaryParentCategoryId', label: 'Primary Parent' },
...
export default function FindCategoryPage() {
  ...
  function buildRequest(targetPage = 0, formState = form) {
    ...
    if (criteriaHasCondition(formState.productCategoryId)) {
      payload.productCategoryId = formState.productCategoryId;
    }
    if (criteriaHasCondition(formState.categoryName)) payload.categoryName = formState.categoryName;
    return payload;
  }
  ...
  async function runSearch(targetPage = 0, formState = form) {
    ...
    setLoading(true);
    try {
      const response = await findCategories(buildRequest(targetPage, formState));
      setResults(response);
  ...
  function ResultsPagination() {
    ...
    return (
      <nav className="results-pagination" aria-label="Category search results pagination">
        ...
        <ul className="pagination-nav">
          ...
          <li className="pagination-pagesize">
            <label htmlFor="category-results-page-size">View</label>
            <select
              id="category-results-page-size"
              value={form.size}
    ...
    <div>
      <div className="screenlet">
        ...
        <div className="screenlet-body">
          <p>Search product categories by category ID or category name (OFBiz FindCategory).</p>
          {error && <div className="alert alert-error">{error}</div>}
          <form onSubmit={handleSubmit}>
            <div className="find-form-grid">
              <SearchCriteriaField
                label="Category ID"
                criteria={form.productCategoryId}
                onChange={(c) => updateCriteria('productCategoryId', c)}
              />
              <SearchCriteriaField
                label="Category Name"
                criteria={form.categoryName}
                onChange={(c) => updateCriteria('categoryName', c)}
              />
            </div>
            <div className="form-field">
              <label className="checkbox-row">
                ...
                />
                List all categories when no search criteria entered (OFBiz noConditionFind)
              </label>
            </div>
      ...
      {results && (
        <div className="screenlet">
          <div className="screenlet-title">Categories</div>
                    ...
                    <tr key={row.productCategoryId}>
                      <td>
                        {canAccess(WRITE_ROLES) ? (
                          ...
                          <strong>{row.productCategoryId}</strong>
                        )}
                      </td>
                      <td>{row.productCategoryTypeId}</td>
                      <td>{row.primaryParentCategoryId}</td>
